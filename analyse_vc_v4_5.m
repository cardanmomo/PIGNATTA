...
%icoil_flux.pc=0;
cp = 1;  % Control parmeter (refer to the table on the experimental guide)
che = 0;  % initializes determinator of config and mastoutline plot
%first make the icoil structure
%dI=3.0; %parameter to multiply the S matrix values
for dI=1:30
    ...
    fclose(fid);
    vals=fopen('dummy.txt', 'a');
    fprintf(fid, '%f', dI);
    for j=1:numel(x_control)
        if j < numel(x_control)
            fprintf(fid, ' %f', delta_vc(cp,j));
        end
        if j == numel(x_control)
         fprintf(fid, ' %f\n', delta_vc(cp,j));
        end
    ...
    %plot the result and check the location of the control points is
    %correct
