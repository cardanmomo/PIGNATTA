    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_fastParallelCalcSensitivityColumn.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * fastParallelCalcSensitivityColumn.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * fastParallelCalcSensitivityColumn.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * fastParallelCalcSensitivityColumn (application created by MATLAB Compiler)
        * run_fastParallelCalcSensitivityColumn.sh (shell script that can be run to 
    ...
    All Platforms:
        * fastParallelCalcSensitivityColumn.so (shared library, file extension varies by 
          platform)
        * fastParallelCalcSensitivityColumn.h (library header file)
        * fastParallelCalcSensitivityColumn.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * fastParallelCalcSensitivityColumn.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * fastParallelCalcSensitivityColumn.xml (documentation files)
        * fastParallelCalcSensitivityColumn.pdb (program database file - if DEBUG option 
          selected)
        * fastParallelCalcSensitivityColumn.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * fastParallelCalcSensitivityColumn.ctf (Component Technology File archive, contains 
          compiled M-code)
        * fastParallelCalcSensitivityColumn_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * fastParallelCalcSensitivityColumn.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * fastParallelCalcSensitivityColumn.ctf (Component Technology File archive, contains 
          compiled M-code)
        * fastParallelCalcSensitivityColumn.xla (Optionally created and placed in
          /home/lpang/work/zame/fastSMat directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
